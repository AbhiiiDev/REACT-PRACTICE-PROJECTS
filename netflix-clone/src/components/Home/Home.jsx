...
const Home = () => {
  ...
  useEffect(() => {
    const fetchUpcoming = async () => {
      {
        const {
          data: { results },
        } = await axios.get(`${url}/movie/${upcoming}?api_key=${api}`); //upcoming movie
    ...
    const fetchRated = async () => {
      const { data } = await axios.get(
        `${url}/movie/${toprated}?api_key=${api}`
      );
    ...
    const genre = async () => {
      const {
        data: { genres },
      } = await axios.get(`${url}/genre/movie/list?api_key=${api}`);
      setGenre(genres);
      console.log(genres);
    };
  ...
  return (
     <section className="home">
           ...
           className="banner"
           style={{
          backgroundImage: popular[3]
            ? `url(${`${imgUrl}/${popular[3].poster_path}`})`
            : "none",
          }}
            >
           {popular[3] && <h1>{popular[3].original_title}</h1>}
            {popular[3] && <p>{popular[3].overview}</p>}
